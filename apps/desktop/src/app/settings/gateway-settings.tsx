...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const acceptSavedConfig = (config: GatewaySettingsState) => {
    ...
    setState(normalized)
    setCertificateFingerprint(normalized.clientCertificate?.fingerprint || '')
    setCertificateIssuer(normalized.clientCertificate?.issuer || '')
    setCertificateSerial(normalized.clientCertificate?.serial || '')
    setCertificateSubject(normalized.clientCertificate?.subject || '')
